...
**Layouts** (`_layouts/`):
- `blog.html` — the homepage (`index.md` uses `layout: blog`). Renders the bio and a list (`ul.post-list`) of `site.posts` — a small thumbnail (only if `post.image` is set, no placeholder fallback), date, title, `#category` tags.
- `post.html` — individual post pages. Renders title/date/tags (plus a larger header thumbnail if `page.image` is set), then `{{content}}`, then loads Mermaid (see below) via an inline `<script type="module">`.
...
**Post thumbnails and in-body images are supported.** A post can set `image: /assets/img/whatever.png` in its front matter; `blog.html` renders it as a small (36px) square next to that post's row, `post.html` renders it as a larger header thumbnail on the post page itself, and — because `image` is also the exact front matter key `jekyll-seo-tag`'s `{% seo %}` looks for — it becomes that post's `og:image`/`twitter:image`, i.e. the thumbnail shown when the post is linked on social media or messaging apps. It's exposed as an `image` field (type `image`) in `.pages.yml` so it's pickable from Pages CMS's media browser (media root: `assets`). It's optional — posts without one just show no thumbnail and no link-preview image, there's no generic placeholder graphic or fallback. Post bodies can embed images normally (`![alt](/assets/img/whatever.png)`); `.content img` in `style.css` handles sizing/border/radius. Contact links in `bio.html` remain plain text (no icon images), and `jemoji` is still not re-enabled — write literal unicode emoji in post bodies instead of `:shortcode:` syntax, since jemoji renders shortcodes as `<img>`. Images live in `assets/img/`; there's no image optimization/resizing pipeline in this repo, so keep files reasonably web-sized before committing them.
...
`{% seo %}` only reads a page's own `image` front matter key for `og:image`/`twitter:image` — it does **not** fall back to `_config.yml`'s `logo`. Posts get theirs "for free" from their `image` front matter (see the note above about post thumbnails). `index.md` and `about.md` have no such front matter of their own, so `_config.yml`'s `defaults` sets `image` for those two paths explicitly to the same avatar URL as `logo`/`_data/author.yml`'s `dp` — this is why that URL is duplicated three times across the config; keep them in sync if the avatar changes. Without it, linking the homepage or about page on social media/WhatsApp would show no preview image at all.
